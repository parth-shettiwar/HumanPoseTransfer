...
from torch.nn import Linear, ReLU, CrossEntropyLoss, Sequential, Conv2d, MaxPool2d, Module, Softmax, BatchNorm2d, Dropout, BatchNorm1d, LeakyReLU, BatchNorm1d
...
class PoseEncoder(nn.Module):
  def __init__(self, lr_negative_slope=0.2, use_bias = True):
    super(PoseEncoder, self).__init__()
    self.negative_slope = lr_negative_slope
    self.CNN_layers = nn.Sequential(
      nn.Conv2d(36, 8, kernel_size=4, stride=2, padding=0, bias=use_bias),
      nn.LeakyReLU(self.negative_slope, inplace=True),

      nn.Conv2d(8, 16, kernel_size=3, stride=1, padding=1, bias=use_bias),
      nn.LeakyReLU(self.negative_slope, inplace=True),
      nn.Conv2d(16, 16, kernel_size=3, stride=2, padding=0, bias=use_bias),
      nn.BatchNorm2d(16),
      nn.LeakyReLU(self.negative_slope, inplace=True),

      nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1, bias=use_bias),
      nn.LeakyReLU(self.negative_slope, inplace=True),
      nn.Conv2d(32, 32, kernel_size=3, stride=2, padding=0, bias=use_bias),
      nn.BatchNorm2d(32),
      nn.LeakyReLU(self.negative_slope, inplace=True),
...
class ImageEncoder(nn.Module):
  def __init__(self, lr_negative_slope=0.2, use_bias = True):
    ...
    self.negative_slope = lr_negative_slope
    self.CNN_layers = nn.Sequential(
      nn.Conv2d(3, 8, kernel_size=4, stride=2, padding=0, bias=use_bias),
      nn.LeakyReLU(self.negative_slope, inplace=True),
...
class G_synthesis(nn.Module):
    def __init__(self,
        ...
        for layer_idx in range(num_layers):
            ...
            self.noise_inputs.append(torch.zeros(*shape).to("cpu"))
    ...
    def forward(self, dlatent_image, dlatent_pose):
        """
        ...
        if self.structure == 'fixed':
            # initial block 0:
            x = self.const_input.expand(dlatent_pose.size(0), -1, -1, -1)
            x = x + self.bias.view(1, -1, 1, 1)
            x = self.adaIn1(x, self.noise_inputs[0], dlatent_pose[:, 0])
            x = self.conv1(x)
            x = self.adaIn2(x, self.noise_inputs[1], dlatent_pose[:, 1])

            # block 1:
            # 4 x 4 -> 8 x 8
            x = self.GBlock1(x, dlatent_pose)

            # block 2:
            # 8 x 8 -> 16 x 16
            x = self.GBlock2(x, dlatent_pose)

            # block 3:
            # 16 x 16 -> 32 x 32
            x = self.GBlock3(x, dlatent_image)

            # block 4:
            # 32 x 32 -> 64 x 64
            x = self.GBlock4(x, dlatent_image)

            # block 5:
            # 64 x 64 -> 128 x 128
            x = self.GBlock5(x, dlatent_image)

            # block 6:
            # 128 x 128 -> 256 x 256
            x = self.GBlock6(x, dlatent_image)

            # block 7:
            # 256 x 256 -> 512 x 512
            x = self.GBlock7(x, dlatent_image)

            # block 8:
            # 512 x 512 -> 1024 x 1024
            x = self.GBlock8(x, dlatent_image)
...
class StyleGenerator(nn.Module):
    def __init__(self,
        ...
        self.mapping_img = G_mapping(self.mapping_fmaps, **kwargs)
        self.mapping_pose = G_mapping(self.mapping_fmaps, **kwargs)
    ...
    def forward(self, image, pose):
        ...
        dlatents_img, num_layers_img = self.mapping_img(latent_image)
        dlatents_pose, num_layers_pose = self.mapping_pose(latent_pose)
        # let [N, O] -> [N, num_layers, O]
        # 这里的unsqueeze不能使用inplace操作, 如果这样的话, 反向传播的链条会断掉的.
        dlatents_pose = dlatents_pose.unsqueeze(1)
        dlatents_pose = dlatents_pose.expand(-1, int(num_layers_pose), -1)

        dlatents_img = dlatents_img.unsqueeze(1)
        dlatents_img = dlatents_img.expand(-1, int(num_layers_img), -1)
        ...
        if self.truncation_psi and self.truncation_cutoff:
            coefs1 = np.ones([1, num_layers_img, 1], dtype=np.float32)
            for i in range(num_layers_img):
                if i < self.truncation_cutoff:
                    coefs1[:, i, :] *= self.truncation_psi
            ...
            """
        if self.truncation_psi and self.truncation_cutoff:
            coefs2 = np.ones([1, num_layers_pose, 1], dtype=np.float32)
            for i in range(num_layers_pose):
                if i < self.truncation_cutoff:
                    coefs2[:, i, :] *= self.truncation_psi
            """Linear interpolation.
               a + (b - a) * t (a = 0)
               reduce to
               b * t
            ...
            dlatents_img = dlatents_img * torch.Tensor(coefs1).to(dlatents_img.device)
            dlatents_pose = dlatents_pose * torch.Tensor(coefs2).to(dlatents_pose.device)

        img = self.synthesis(dlatents_img, dlatents_pose)
        return img
